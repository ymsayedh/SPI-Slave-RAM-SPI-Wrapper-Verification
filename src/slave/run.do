...
set design {src/slave/SPI_slave.v slave}
...
proc start {args} {  
    ...
    if {[lsearch -exact $args -wavefile] >= 0} {
        ...
        add wave /[lindex $design 1]_top/[lindex $design 1]_iff/*
        radix unsigned
    } else {
        add wave *
        radix unsigned
    ...
    if {[lsearch -exact $args -cover_report] >= 0} {
        ...
        coverage report -detail -cvg -directive -comments -option -output reports/slave_fcover_report.txt
        echo "Wrote the function coverage report in reports/slave_fcover_report.txt"
        coverage report -output reports/slave_codecover_report.txt -du=SLAVE -detail -all -dump -annotate -assert -directive -cvg -codeAll
        echo "Wrote the code coverage report in reports/slave_codecover_report.txt"
